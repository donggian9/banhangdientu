...
namespace QuanLyBanHangDienTu.Presentation
{
    public partial class fr_Donvitinh : Form
    {
        ...
        E_tb_Donvitinh thucthi = new E_tb_Donvitinh();
        ConnectDB cn = new ConnectDB();
        EC_tb_Donvitinh ck = new EC_tb_Donvitinh();
        bool themmoi;
        int dong = 0;
        ...
        public void khoitaoluoi()
        {
            ...
            msds.Columns[0].HeaderText = "Mã Đơn Vị";
            msds.Columns[0].Frozen = true;
            msds.Columns[0].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;
            msds.Columns[0].Width = 150;
            msds.Columns[1].HeaderText = "Tên Đơn Vị";
            msds.Columns[1].Width = 200;

        }
        public void hienthi()
        {
            string sql = "SELECT madonvi, tendonvi FROM tb_Donvitinh";
        ...
        private void btluu_Click(object sender, EventArgs e)
        {
            if (txtma.Text != "")
            {
                if (themmoi == true)
                {
                    try
                    {
                        ck.MADONVI = txtma.Text;
                        ck.TENDONVI = txtten.Text;
                        thucthi.themoil(ck);
                        locktext();
                        hienthi();
